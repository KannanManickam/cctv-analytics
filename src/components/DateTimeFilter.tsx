...
const DateTimeFilter: React.FC<DateTimeFilterProps> = ({ onChange, className }) => {
  ...
  const handlePresetSelect = (presetId: string) => {
    ...
    switch (presetId) {
      case "today":
        newRange = { from: today, to: new Date(), preset: presetId };
        break;
      case "yesterday":
        ...
        newRange = { from: yesterday, to: yesterdayEnd, preset: presetId };
        break;
      case "7days":
        const sevenDaysAgo = new Date(today);
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
        newRange = { from: sevenDaysAgo, to: new Date(), preset: presetId };
        break;
      case "30days":
        const thirtyDaysAgo = new Date(today);
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        newRange = { from: thirtyDaysAgo, to: new Date(), preset: presetId };
        break;
      case "90days":
        const ninetyDaysAgo = new Date(today);
        ninetyDaysAgo.setDate(ninetyDaysAgo.getDate() - 90);
        newRange = { from: ninetyDaysAgo, to: new Date(), preset: presetId };
        break;
      case "ytd":
        const startOfYear = new Date(today.getFullYear(), 0, 1);
        newRange = { from: startOfYear, to: new Date(), preset: presetId };
        break;
      case "custom":
